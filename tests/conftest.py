...
MANIFEST = {m["scenario"]: m for m in json.loads((config.SAMPLES_DIR / "manifest.json").read_text(encoding="utf-8"))}
...
def sample(scenario: str) -> tuple[Path, InvoiceData]:
    ...
    pdf = config.SAMPLES_DIR / "invoices" / item["file"]
